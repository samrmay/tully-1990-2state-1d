...
class FSSH_1d:
    ...
    def __init__(self, potential_model, del_t, x0, v0, m=2000, t0=0,
        ...
        self.coeff = coeff0
        c1, c2 = coeff0
        self.d_mtx = np.asarray([[c1*(c1.conjugate()), c1*(c2.conjugate())],
    ...
    def get_density_mtx_3(self, x0, v0, e_state, t0=0):
        def f(t, c):
            x, dx = self.calc_trajectory(x0, self.m, v0, t, e_state)
            _, e_functions = self.get_electronic_state(x)
            nacvs = self.get_NACV(x, e_functions)
            ...
            ih_bar = 1j*self.HBAR
            mtx = np.asarray([[c[0], c[1]], [c[2], c[3]]], dtype=complex)
            result = np.zeros(self.num_states**2, dtype=complex)
            i = -1
            for k in range(self.num_states):
                for j in range(self.num_states):
                    ...
                    for l in range(self.num_states):
                        result[i] += mtx[l, j] * \
                            (V[k, l] - ih_bar*np.dot(x, nacvs[k, l]))
                        result[i] -= mtx[k, l] * \
                            (V[l, j] - ih_bar*np.dot(x, nacvs[l, j]))

            return result/ih_bar

        result = integrate.solve_ivp(f, (t0, self.del_t), self.d_mtx.flatten())
        a11, a12, a21, a22 = result.y[:, -1]
        self.d_mtx = np.asarray([[a11, a12], [a21, a22]], dtype=complex)
        return self.d_mtx
    ...
    def run(self, max_step, stopping_function, debug=False):
        ...
        for i in range(max_step):
            ...
            d_mtx = self.get_density_mtx_3(x0, v0, e_state0)
            self.d_mtx = d_mtx
            print(d_mtx[0, 0], d_mtx[1, 1], d_mtx[0, 0] + d_mtx[1, 1])
